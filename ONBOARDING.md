...
can start tomorrow. Ask conversationally, a few questions at a time rather than a form
...
1. **Write `ATHLETE.md`** in the repo root: goal + race, background, benchmarks,
   ...
   This file is the coach's memory, so keep it current as things change.
...
3. **Set up the data pipeline** with the athlete (`docs/SETUP-HEALTHFIT.md`), run
   ...
   historical workouts, ingest them, because history calibrates everything.
...
5. **Write week 1's plan** using the output format in `CLAUDE.md`. Start
   conservative: the first two weeks establish a baseline and build trust, and the
